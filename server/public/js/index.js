				...
				self.countdown(res.end_time, function (time) {
					...
					setTimeout(function () {
						...
						self.getOpenCode(self.issue).then(function (res) {
							// console.log("opencode",res.data);从服务器获得了opencode
							var data = res.data;
							// console.log(data);
							data = data.map(function (item) {
								return item.padStart(2, "0");
							});
							var oLi = document.getElementsByClassName("ico-ball3");
							for (var i = 0; i < oLi.length; i++) {
								oLi[i].innerHTML = data[i];
							}
						});
					}, 500);
				});
...
var Base = function () {
   ...
   */
		value: function initPlayList() {
			this.play_list.set('r2', {
				...
				bonus: 19,
				tip: "从01～11中任选3个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>19</em>元",
				name: '任三'
			}).set('r4', {
				bonus: 78,
				tip: "从01～11中任选4个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>78</em>元",
				name: '任四'
			}).set('r5', {
				bonus: 540,
				tip: "从01～11中任选5个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>540</em>元",
				name: '任五'
			}).set('r6', {
				bonus: 90,
				tip: "从01～11中任选6个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>90</em>元",
				name: '任六'
			}).set('r7', {
				bonus: 26,
				tip: "从01～11中任选7个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>26</em>元",
				name: '任七'
			}).set('r8', {
				bonus: 9,
				tip: "从01～11中任选3个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class='red'>9</em>元",
				name: '任八'
			});
		}
   ...
   */
		...
		value: function addCodeItem(code, type, typeName, count) {
			var self = this;
			var tpl = '\n\t\t<li count="' + count + '" bonus="' + count * 2 + '" codes="' + type + '|' + code + '">\n\t\t\t <div class="code">\n\t\t\t\t <b>' + typeName + (count > 1 ? '复式' : '单式') + '</b>\n\t\t\t\t <b class="em">' + code + '</b>\n\t\t\t\t [' + count + '\u6CE8,<em class="code-list-money">' + count * 2 + '</em>\u5143]\n\t\t\t </div>\n\t \t</li>';
		...
		value: function getCount() {
			...
			// console.log(count,range,win1,win2);
			if (count === 0) {
...
var Calculate = function () {
   ...
   * @param  {number} size [组合运算的基数，也是选择玩法的数字]
   * @return {number}      [计算注数]
   */
